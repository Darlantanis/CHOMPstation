...
CHOMP EDIT: Legacy description kept to easily reference the original design. Bits of code will be commented out for features not (yet) supported/desired here. Like the loot.
*/

/mob/living/simple_animal/animals/megafauna_ch/ashdrake
	name = "ash drake"
	desc = "A terrifying, ravenous beast."
	health = 2500
	maxHealth = 2500
	attacktext = "chomps"
	attack_sound = 'sound/magic/demon_attack1.ogg'
	icon = 'icons/mob/64x64.dmi'
	icon_state = "AshDragon"
	icon_living = "AshDragon"
	icon_dead = "AshDragon-dead"
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/Process_Spacemove(var/check_drift = 0)
	return 1

/* /mob/living/simple_animal/animals/megafauna_ch/ashdrake/Initialize()
	smallsprite.Grant(src)
	. = ..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/death()
	QDEL_NULL(internal) // so drake corpses don't have a gps signal
	. = ..()
*/

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/ex_act(severity, target)
	if(severity == 3)
		return
	..()
/*
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(!forced && (swooping & SWOOP_INVULNERABLE))
		return FALSE
	return ..()
*/
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/visible_message()
	if(swooping & SWOOP_INVULNERABLE) //to suppress attack messages without overriding every single proc that could send a message saying we got hit
		return
	return ..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/AttackingTarget()
	if(!swooping)
		return ..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/DestroySurroundings()
	if(!swooping)
		..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/Move()
	if(!swooping)
		..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/Goto(target, delay, minimum_distance)
	if(!swooping)
		..()

/mob/living/simple_animal/animals/megafauna_ch/ashdrake/OpenFire()
	if(swooping)
		return
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/lava_pools(var/amount, var/delay = 0.8)
	if(!target)
		return
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/lava_swoop(var/amount = 30)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/mass_fire(var/spiral_count = 12, var/range = 15, var/times = 3)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/lava_arena()
	if(!target)
		return
	...
	for(var/turf/T in RANGE_TURFS(2, center))
		...
		if(!istype(T, /turf/unsimulated))
			T.ChangeTurf(/turf/simulated/mineral/floor)
		else
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/arena_escape_enrage() // you ran somehow / teleported away from my arena attack now i'm mad fucker
	SetRecoveryTime(80)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/fire_cone(var/atom/at = target)
	playsound(get_turf(src),'sound/magic/fireball.ogg', 200, 1)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/line_target(var/offset, var/range, var/atom/at = target)
	if(!at)
		return
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/fire_line(var/list/turfs)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/proc/swoop_attack(lava_arena = FALSE, atom/movable/manual_target, var/swoop_cooldown = 30)
	if(stat || swooping)
		return
	if(manual_target)
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
...
/mob/living/simple_animal/animals/megafauna_ch/ashdrake/death()
	..()
	visible_message("[src] collapses into a pile of bones, its flesh sloughing away.")
...
/obj/effect/temp_visual/lava_warning/initialize(mapload, var/reset_time = 10)
	. = ..()
	INVOKE_ASYNC(src, .proc/falling, reset_time)
	src.alpha = 63.75
	animate(src, alpha = 255, time = duration)

/obj/effect/temp_visual/lava_warning/proc/falling(var/reset_time)
	var/turf/T = get_turf(src)
	...
	for(var/mob/living/L in T.contents)
		if(istype(L, /mob/living/simple_animal/animals/megafauna_ch/ashdrake))
			continue
		L.adjustFireLoss(10)
	...
	if(!istype(T, /turf/unsimulated) && !istype(T, /turf/simulated/floor/lava))
		var/lava_turf = /turf/simulated/floor/lava/smooth
